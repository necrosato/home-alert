...
reverse proxy, forwarding requests to the proper main server. The control
...
control server to the protected main server.  This is needed for three reasons,
...
Users should not be able to access the main servers directly. For security
reasons, the only things that can talk to a main server are a trigger or the
...
This should be able to resolve names like home-alert.location to main server ip
